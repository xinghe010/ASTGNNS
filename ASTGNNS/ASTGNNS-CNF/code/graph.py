...
#            for child in node.children:
#                if child in function_nodes and child.id > node.id:
#                    child.children.append(node)
#                    node.parents.append(child)
#                if child in variable_nodes:
#                    child.children.append(node)
#                    node.parents.append(child)

#        for node in function_nodes:
#            for parent in node.parents:
#                if not re.match(VARIABLE_PATTERN, parent.name):
#                    if parent.id > node.id:
#                        for parent1 in parent.children:
#                            if re.match(VARIABLE_PATTERN, parent1.name):
#                                node.parents.append(parent1)
#                                parent1.children.append(node)
#                            if re.match(FUNCTOR_PATTERN, parent1.name) and parent1.id > parent.id:
#                                node.parents.append(parent1)
#                                parent1.children.append(node)

#        for node in quantifer_nodes:
#            for child in node.children:
#               node.parents.append(child)
#                child.children.append(node)

        #for node in connective_nodes1:
         #   for index in enumerate(node.children):
...
#A = fof_formula_transformer("fof(t5_xboole_0, axiom, (![C]:(![B]:(![A]:(~r1_xboole_0(A,B)|(~r2_hidden(C,k2_xboole_0(A,B))|(r2_hidden(C,A)|(r2_hidden(C,B)))))&((~r1_xboole_0(A,B)|(~r2_hidden(C,k2_xboole_0(A,B))|(r2_hidden(C,A)|(~r2_hidden(C,A)))))&((~r1_xboole_0(A,B)|(~r2_hidden(C,k2_xboole_0(A,B))|(~r2_hidden(C,B)|(r2_hidden(C,B)))))&((~r1_xboole_0(A,B)|(~r2_hidden(C,k2_xboole_0(A,B))|(~r2_hidden(C,B)|(~r2_hidden(C,A)))))))))))).")
#print(Graph(A).graph)
#Draw(Graph(A))
